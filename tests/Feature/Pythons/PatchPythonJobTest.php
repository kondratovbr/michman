...
use App\Scripts\Root\Python3_9\PatchPythonScript;
use Illuminate\Support\Facades\Bus;
use Illuminate\Support\Facades\Event;
use Mockery\MockInterface;
use Tests\AbstractFeatureTest;

class PatchPythonJobTest extends AbstractFeatureTest
{
    public function test_job_parameters_and_logic()
    {
        ...
        $python = Python::factory([
            'version' => '3_9',
        ])->withServer()->updating()->create();
        $server = $python->server;

        $this->mockBind(PatchPythonScript::class, function (MockInterface $mock) use ($server) {
            $mock
        ...
        Event::assertDispatched(PythonPatchedEvent::class);
    }
}
